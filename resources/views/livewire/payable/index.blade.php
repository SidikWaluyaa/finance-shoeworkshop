        ...
        <div class="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div class="bg-slate-900 text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-6 border border-slate-800">
                ...
                <div class="flex items-center gap-2">
                    <button wire:click="bulkDelete" wire:confirm="Hapus {{ count($selectedRows) }} data utang terpilih?" class="flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-red-500/10 text-red-400 transition-colors text-sm font-medium">
            ...
            <table class="data-table">
                ...
                <tbody>
                    @forelse($payables as $pay)
                        <tr wire:key="pay-{{ $pay->id }}" class="{{ in_array($pay->id, $selectedRows) ? 'bg-primary-50/30 dark:bg-primary-900/10' : '' }}">
                            <td>
                                <div class="flex items-center justify-center">
                                    <input type="checkbox" wire:model.live="selectedRows" value="{{ $pay->id }}" class="form-checkbox h-4 w-4 rounded border-slate-300 dark:border-slate-600 text-[var(--color-primary)] focus:ring-[var(--color-primary)]">
                                </div>
                            </td>
                            <td>

<div x-data="{ show: @entangle('showModal') }" x-show="show" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
    <div class="flex items-center justify-center min-h-screen p-4 px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" 
            class="inline-block relative overflow-hidden text-left align-bottom transition-all transform bg-white dark:bg-[#1e293b] rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border dark:border-slate-800">
            
            <div class="px-6 pt-6 pb-4">
                <div class="flex items-center justify-between mb-6">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Import Transaksi Massal</h3>
                    <button @click="show = false" class="text-gray-400 hover:text-gray-500 dark:hover:text-white transition-colors">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                    </button>
                </div>

                {{-- Template Download Section --}}
                <div class="bg-primary/5 dark:bg-emerald-500/5 rounded-2xl p-5 mb-6 border border-primary/10 dark:border-emerald-500/20 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 bg-primary/10 dark:bg-emerald-500/20 rounded-xl flex items-center justify-center text-primary dark:text-emerald-400">
                        ...
                        <div>
                            <p class="font-bold text-gray-900 dark:text-white">Belum punya template?</p>
                            <p class="text-xs text-gray-500 dark:text-slate-400">Unduh template Excel dengan dropdown & validasi pintar.</p>
                        </div>
                    </div>
                    <a href="{{ route('transactions.template') }}" class="bg-white dark:bg-slate-800 text-primary dark:text-emerald-400 border border-primary/20 dark:border-emerald-500/30 px-4 py-2 rounded-xl text-sm font-bold hover:bg-primary dark:hover:bg-emerald-500 hover:text-white transition-all shadow-sm">
                        Unduh Template
                    </a>
                </div>
                ...
                <div class="relative group">
                    ...
                    <div class="border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-2xl p-8 text-center group-hover:border-primary/50 dark:group-hover:border-emerald-500/50 group-hover:bg-primary/5 dark:group-hover:bg-emerald-500/5 transition-all {{ $file ? 'bg-green-50 dark:bg-emerald-900/10 border-green-200 dark:border-emerald-500/30' : '' }}">
                        <div class="mx-auto w-16 h-16 bg-gray-50 dark:bg-slate-800 rounded-2xl flex items-center justify-center mb-4 text-gray-400 group-hover:text-primary dark:group-hover:text-emerald-400 transition-colors">
                            @if($file)
                ...
                <div class="mt-6">
                    <p class="text-sm font-bold text-red-600 dark:text-rose-400 mb-3 flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                        Beberapa data gagal divalidasi:
                    </p>
                    <div class="max-h-48 overflow-y-auto bg-red-50 dark:bg-rose-950/20 rounded-2xl border border-red-100 dark:border-rose-900/30 p-4 space-y-3">
                        @foreach($importFailures as $failure)
                        <div class="text-xs text-red-700 dark:text-rose-300 pb-2 border-b border-red-100 dark:border-rose-900/30 last:border-0 last:pb-0">
            ...
            <div class="px-6 py-4 bg-gray-50 dark:bg-slate-900/50 border-t border-gray-100 dark:border-slate-800 flex items-center justify-end gap-3 rounded-b-3xl">
